...
class QuestionSpec(BaseModel):
    ...
    def validate_options(self) -> QuestionSpec:
        ...
        if not self.options or len(self.options) < 2:
            raise ValueError(
                f"{self.type.value} requires at least two options"
            )
        if len(self.options) > 255:
            raise ValueError(
                "v0 caps option cardinality at 255"
            )
...
class TargetSpec(BaseModel):
    ...
    def validate_target(self) -> TargetSpec:
        if (
            ...
            raise ValueError(
                "target needs index or distribution"
            )
        if self.distribution is not None:
            ...
            if any(
                ...
                raise ValueError(
                    "distribution probabilities must be >= 0"
                )
            total = sum(self.distribution)
            if total <= 0:
                raise ValueError(
                    "distribution must have positive mass"
                )
            self.distribution = [
                p / total
                for p in self.distribution
...
class DecisionRecord(BaseModel):
    ...
    def validate_targets(self) -> DecisionRecord:
        if not self.questions:
            raise ValueError(
                "record must contain at least one question"
            )
        ...
        if unknown:
            raise ValueError(
                "targets reference unknown questions: "
                f"{sorted(unknown)}"
            )

        for name, target in self.targets.items():
            size = len(
                self.questions[name].options
                or []
            )
            if (
                ...
                raise ValueError(
                    f"target index out of range for {name}"
                )
            if (
                target.distribution is not None
                and len(
                    target.distribution
                )
                != size
                ...
                raise ValueError(
                    "target distribution length mismatch "
                    f"for {name}"
                )
        return self
